...
function getPHCStock(phcName) {
  try {
    ...
    data.forEach(row => {
      ...
      const currentParsed = row.LastUpdated ? parseDateFlexible(row.LastUpdated) : null;
      const currentTime = currentParsed ? currentParsed.getTime() : 0;
      if (!existing) {
        latestByMedicine.set(key, row);
      } else {
        const existingParsed = existing.LastUpdated ? parseDateFlexible(existing.LastUpdated) : null;
