...
// --- CONFIGURATION ---
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzSZ7xsjyDPLAEvjUq3dKsdKvxBKIx93X0BVmTvZbj4_q9gV5sDWVE0IQqcJSV7zi9b/exec';
...
// Flexible date parser used throughout the app. Accepts dd/mm/yyyy, dd-mm-yyyy, ISO (yyyy-mm-dd),
// or a Date object. Returns a Date normalized to local midnight or null if invalid.
function parseFlexibleDate(val) {
    ...
    if (/^\d{4}-\d{2}-\d{2}/.test(s)) {
        // Ensure time component to avoid timezone shifts when using toISOString
...
async function initializeDashboard() {
    ...
    try {
        ...
        showLoader('Loading patient and follow-up data...');
        ...
        // Defer admin-only operations to run after UI is responsive
        setTimeout(async () => {
...
function initializeAllCharts() {
    ...
    { id: 'residenceChart', fn: () => renderPieChart('residenceChart', 'Residence Type', activePatients.map(p => (p.ResidenceType || 'Unknown').toString().trim() || 'Unknown')) },
        ...
        { id: 'ageChart', fn: () => renderHistogramChart('ageChart', 'Patient Age Distribution', activePatients.map(p => p.Age)) },
        { id: 'ageOfOnsetChart', fn: () => renderHistogramChart('ageOfOnsetChart', 'Age of Onset Distribution', activePatients.map(p => p.AgeOfOnset)) }
    ];
...
function renderHistogramChart(canvasId, title, dataArray, binSize = 10) {
    if (!Array.isArray(dataArray)) {
        console.warn(`renderHistogramChart: expected array for ${canvasId} but got`, dataArray);
        dataArray = [];
    }
    // Filter out non-numeric values
    const numericData = dataArray.filter(val => {
        const num = parseFloat(val);
        return !isNaN(num) && isFinite(num) && num >= 0;
    }).map(val => parseFloat(val));
    
    if (numericData.length === 0) {
        console.log(`No valid numeric data for ${title}`);
        return;
    }
    ...
    for (let i = 0; i <= Math.ceil(max / binSize); i++) {
        const binStart = i * binSize;
        const binEnd = (i + 1) * binSize - 1;
        bins.push({ start: binStart, end: binEnd, count: 0 });
        labels.push(binEnd < max ? `${binStart}-${binEnd}` : `${binStart}+`);
    }
    ...
    numericData.forEach(val => {
        ...
        if (binIndex < bins.length) {
            ...
            // For values beyond the last bin
    ...
    renderChart(canvasId, 'bar', title, labels, [counts], {
        datasets: [{
            label: 'Count',
...
function renderFollowUpPatientList(phc, searchQuery = '') {
    ...
    patientsForFollowUp.forEach(p => {
        ...
        // Extract completion month and next follow-up date (use robust parser to avoid timezone shifting)
        ...
        if (isCompleted && p.FollowUpStatus) {
            ...
            // Calculate next follow-up date using parseFlexibleDate
            const last = parseFlexibleDate(p.LastFollowUp);
            if (last) {
                const nd = new Date(last.getFullYear(), last.getMonth() + 1, last.getDate());
                // If day doesn't exist in next month (e.g., 31 -> shorter month), JS adjusts; normalize to that date
                nextFollowUpDate = new Date(nd.getFullYear(), nd.getMonth(), nd.getDate()).toISOString().split('T')[0];
            } else {
...
function checkIfFollowUpNeedsReset(patient) {
    // Return false if there's no valid last follow-up date or not completed
    ...
    // Start showing 5 days before the due date and continue thereafter (so overdue items stay flagged)
    ...
    try {
        ...
        allPatients.forEach(patient => {
            const row = [
                ...
                patient.PatientName || '',
                patient.PHC || '',
                patient.NearestAAMCenter || '',
                patient.Phone || '',
                patient.Address || ''
            ];

            // For each month, check if follow-up was done
            months.forEach(month => {
                const monthStart = new Date(month.year, month.month, 1);
                const monthEnd = new Date(month.year, month.month + 1, 0, 23, 59, 59);

                // Find follow-ups for this patient in this month
                const patientFollowUps = followUpsData.filter(followUp => {
                    if (String(followUp.PatientID || '') !== String(patient.ID || '')) return false;
                ...
                if (patientFollowUps.length > 0) {
                    // Get the most recent follow-up for this month
                    const latestFollowUp = patientFollowUps.sort((a, b) =>
                        new Date(b.FollowUpDate) - new Date(a.FollowUpDate)
                    )[0];

                    const submittedBy = latestFollowUp.SubmittedBy ||
                                      latestFollowUp.submittedBy ||
                                      latestFollowUp.SubmittedByUsername ||
                                      latestFollowUp.submittedByUsername ||
                                      'Unknown';

                    row.push(`Followup done by ${submittedBy}`);
                } else {
                    row.push('follow up not done');
                }
            });
        ...
        let csvContent = "data:text/csv;charset=utf-8," + Papa.unparse(csvData);
        const encodedUri = encodeURI(csvContent);
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", `monthly_followup_status_${new Date().toISOString().slice(0,10)}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);

        showNotification('Monthly follow-up status CSV exported successfully!', 'success');

    } catch (error) {
        console.error('Error exporting monthly follow-up status CSV:', error);
...
async function checkAndMarkInactiveByDiagnosis() {
    ...
    if (markedCount > 0) {
        ...
        // Update backend for marked patients - batch the requests
        try {
            ...
            for (let i = 0; i < inactivePatients.length; i += batchSize) {
                ...
                await Promise.all(batch.map(patient => 
                    fetch(SCRIPT_URL, {
                        method: 'POST',
                        mode: 'no-cors',
                        headers: { 'Content-Type': 'application/json' },
